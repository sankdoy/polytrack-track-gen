...
export function decodePolyTrack1(codeRaw) {
...
export function blockName(id) {
  return BLOCK_NAMES[id] || String(id);
}

export function summarize(decoded) {
...
export function stablePartKey(p) {
  const ck = p.checkpointOrder ?? "";
  const so = p.startOrder ?? "";
  return `${p.blockType},${p.x},${p.y},${p.z},${p.rotation},${p.rotAxis},${p.color},${ck},${so}`;
}

export function stablePartCoreKey(p) {
  // Ignore rotAxis/color when trying to match geometry moves.
  const ck = p.checkpointOrder ?? "";
  const so = p.startOrder ?? "";
  return `${p.blockType},${p.x},${p.y},${p.z},${p.rotation},${ck},${so}`;
}
